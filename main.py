...
description = "BOT feito exclusivo para a LIF, por: Wick19. Uso somente da Staff. \nDigite `/help` para ver seus comandos!"
## Prefix
client = commands.Bot(command_prefix=commands.when_mentioned_or('/'), case_insensitive = True, description = description, hidden=True, intents=intents)
...
async def on_ready():
    print('----------------------------------------------------')
    print(f'Entrou como {client.user}')                     ## USERNAME
    print(f'bot_id: {client.user.id}')                      ## USER ID
    print(f"discord.py version: {discord.__version__}")     ## DISCORD VERSION
    print(f"LIF BOT version: {version}")                    ## Your bot version
    print('----------------------------------------------------')
...
async def change_presece():
    ...
    statuses = ['Pai tá on B) \n Wick19', 'Use /liberar_dm, no: #comandos-do-bot para liberar a DM.','Use /help | LIFBOT \n Wick19', 'Use /help | LIFBOT \n Wick19']
...
## Remove the defaut /help, command
...
# Login
@client.command(name='login', aliases= ['entrar', 'loggar'])
...
async def help(ctx):
    await ctx.send(embed=discord.Embed(
        title = 'HELP!',
        url = 'https://github.com/RickDB17/LIFBOT/blob/main/HELP.md',
        description = 'Clique no título e seja enviado à página de ajuda.',
        color = 0xffdd00
    ))

## Add an role to all users in guild
...
## Send a random joke! :D
@client.command(name='piada', aliases = ['façamerir'])
...
## "I want" System
@client.group(name='pedir', aliases=['quero'])
@commands.has_any_role(ativo)
async def pedir(ctx):
...
@carta.command(name='guild', aliases=['server','servidor','canal','channel', 'ch'])
async def sv_subcmd(ctx, channel:discord.TextChannel, *, message):
    await ctx.channel.purge(limit=1)
    msg = discord.Embed(title=f'{ctx.guild.name}, {ctx.message.created_at.day}/{ctx.message.created_at.month}/{ctx.message.created_at.year}',
                        description=f'```{message}```',
                        color=0xffdd00)
...
# ADM AVISOS
@client.group(name='mural', aliases = ['notificação', 'notificaçao', 'notif', 'notificar'])
@commands.has_any_role(staff)
async def mural(ctx):
    msg = discord.Embed(title=f'Escrever Notificação',
                        description=f'Para escrever, use:',
                        color=0xffdd00)
    msg.add_field(name='NOTIFICAÇÃO',
                  value='/avisos aviso [mensagem]')
    msg.add_field(name='VOTAÇÃO',
                  value='/avisos votação [mensagem]')
    msg.add_field(name='NOVIDADES',
                  value='/avisos novidades [mensagem]')

    msg.set_footer(text=f'[] para argumentos obrigatórios, <> para opicionais e () explicação. PS.: No comando de votação, você deve usar aspas para delimitar os argumentos.')
    await ctx.send(embed=msg)

@mural.command(name='aviso', aliases=['avisar','avisos'])
async def aviso(ctx, *, message):
    ch = client.get_channel(696132636829155439)
    await ctx.channel.purge(limit=1)
    msg = discord.Embed(title=f'AVISO!',
                        description=f'{message}',
                        color=0xffdd00)
    msg.set_author(name=f'{ctx.author}',
                   icon_url=f'{ctx.author.avatar_url}')
...
@mural.command(name='votação', aliases=['votar','escolhas'])
async def votação(ctx, *, message):
    ch = client.get_channel(696132636829155439)
    await ctx.channel.purge(limit=1)
    
    msg = discord.Embed(title='VOTAÇÃO!',
                        description=f'{message}',
                        color=0xffdd00)
    msg.set_author(name=f'{ctx.author}',
                   icon_url=f'{ctx.author.avatar_url}')
    ...
    await ch.send('@everyone')
    m = await ch.send(embed=msg)
    await ctx.message.add_reaction('\N{WHITE HEAVY CHECK MARK}')

    await m.add_reaction('\N{REGIONAL INDICATOR SYMBOL LETTER A}')
    await m.add_reaction('\N{REGIONAL INDICATOR SYMBOL LETTER B}')
 

@mural.command(name='novidade', aliases=['novidades','news'])
async def novidade(ctx, *, message):
    ch = client.get_channel(696132718853095535)
    await ctx.channel.purge(limit=1)
    msg = discord.Embed(title=f'NOVIDADES!',
                        description=f'{message}',
                        color=0xffdd00)
    msg.set_author(name=f'{ctx.author}',
                   icon_url=f'{ctx.author.avatar_url}')
...
async def warn(ctx):
    ...
    aviso.add_field(name= 'DESBANIR - DESATIVADO', value= '--/warn ban @user motivo--')
...
"""
# UNBan
@warn.command(name = 'unban', aliases = ['desbanir','unb'], description = 'Na reason, escreva no infinitivo.')
...
async def det(ctx, member: discord.Member, role: discord.Role, *, reason):
    ...
    if r_ativos in member.roles:
        await ctx.send(f"{member} tinha {r_ativos}")
        await member.remove_roles(r_ativos)
        log.add_field(name="Cargo removido:",
        value=f"{r_ativos.mention}",
        inline=True)
...
@warn.command(name = 'desdet', aliases= ['liberar', 'libertar', 'soltar'])
...
@client.command(name='liberar_dm', aliases = ['dm', 'privado'])
